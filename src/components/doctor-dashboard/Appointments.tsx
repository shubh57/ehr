...
const Appointments: React.FC = () => {
    ...
    const fetchAppointments = async () => {
        try {
            ...
            const data: Appointment[] = await invoke('get_appointments', { token });
    ...
    const fetchDoctors = async () => {
        try {
            const data: UserInterface[] = await invoke('get_all_doctors', { token });
            setDoctors(data.filter(doctor => doctor.user_id !== user?.user_id));
        } catch (error) {
    ...
    const createAppointment = async () => {
        try {
            ...
            console.log("appointment_creation: ", data);
            setDescription("");
    ...
    const getDoctorAppointments = (doctorIds: number[]) => {
        return appointments.filter(app => 
            doctorIds.includes(app.doctor_id)
        ).map(app => ({
    ...
    return (
        <Box sx={{ p: 3 }}>
            ...
            <Paper elevation={3} sx={{ p: 3, mb: 3, maxHeight: '12rem', overflowY: 'scroll' }}>
                <Typography variant="h6" gutterBottom>Scheduled Appointments</Typography>
                {appointmentsLoading ? (
                    <CircularProgress />
                ) : (
                    <List>
                ...
                <Paper elevation={3} sx={{ 
                    ...
                    <FormControl fullWidth sx={{ mb: 2 }}>
                        ...
                        <Select
                            ...
                            onChange={(e) => setSelectedDoctors(e.target.value as number[])}
                        >
                            {doctors.map((doctor) => (
                    ...
                    {appointmentTime && (
                        <Typography sx={{ mb: 2 }}>
                            Selected Time: {appointmentTime.toLocaleString('en-IN')}
                        </Typography>
                    )}
