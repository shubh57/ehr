...
import { Box, Typography, List, ListItem, ListItemText, ListItemAvatar, ListItemButton, Avatar, TextField, IconButton, Divider, Badge, useTheme } from '@mui/material';
    ...
    return (
        <Box sx={{ display: 'flex', height: '100%', borderRadius: 2, overflow: 'hidden', boxShadow: 3, maxHeight: '40rem' }}>
            ...
            <Box sx={{ width: 300, borderRight: 1, borderColor: 'divider', display: 'flex', flexDirection: 'column' }}>
                ...
                <Divider />
                <Typography variant="subtitle1" sx={{ p: 2 }}>Start New Chat</Typography>
            ...
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', bgcolor: 'background.paper' }}>
                {selectedConversation ? (
                    <>
                        <Box sx={{ p: 2, borderBottom: 1, borderColor: 'divider', display: 'flex', alignItems: 'center' }}>
                            ...
                            <Typography variant="h6">
                                {`Dr. ${getRecipientName(selectedConversation)}`}
                            </Typography>
                        </Box>
                        <Box sx={{ 
                            flex: 1, 
                            ...
                            {conversationLoading ? (
                                <Typography align="center" sx={{ color: 'text.secondary', py: 2 }}>Loading messages...</Typography>
                            ) : messages.length === 0 ? (
                                <Typography align="center" sx={{ color: 'text.secondary', py: 2 }}>No messages yet. Start the conversation!</Typography>
                            ) : (
                                messages.map((msg) => {
                    ...
                    <Box sx={{ 
                        ...
                        <Typography variant="h6" color="text.secondary" gutterBottom>No conversation selected</Typography>
                        <Typography variant="body1" color="text.secondary">
                            Select a conversation or start a new one with a doctor from the list
                        </Typography>
                    </Box>
                )}
            </Box>
